...
前端吃的是**扁平列**（一列 = 一個「品牌 × 材質 × 工法 × 顏色」的組合）。但底層真實世界是**多對多**的：同一個色號會跨多個專案、多個材質、多個供應商。所以：
...
**後果**：無法回答「所有做陽極的件有哪些」——因為陽極這個資訊，一半在工法欄、一半在備註欄。

→ Canonical schema 拆成 **`forming_process`（成型）** 與 **`finish_process`（表面處理）** 兩個獨立欄位。色號後綴（`PE`/`E`/`T`/`P`）對照的是 **`finish_process`，不是 forming**。`scripts/normalize_cmf.py` 會自動判斷欄位裡放的是哪一種，並從 Remark 把表面處理挖出來。
...
| 欄位 | 分隔符 | 陷阱 |
|---|---|---|
| `Project` | `,` `;` `/` | **`M7 1U/2U` 的 `/2U` 是尺寸不是專案**，切完要把 `^\d+U$` 的碎片黏回去 |
...
- `assets/schema.sql` — 可直接貼進 Supabase SQL Editor 的建表語法
